...
const loadServerNotifications=async()=>{
  try{
    ...
    setServerNotifications(data.data||[]);
  }catch(error){
...
const markNotificationAsRead = async (id:string) => {
  ...
  if(id.startsWith('server-')){
    ...
    try{
      await request(`/notifications/${serverId}/read`,{
        method:'PATCH'
      });

      setServerNotifications(previous =>
        previous.map(notification =>
          String(notification.id) === serverId
...
function DemandTable({demands,remove,approve,history,canEdit,canApprove,onEdit,isClient,clients}:{demands:Demand[];remove:(id:string)=>void;approve:(d:Demand,approved?:boolean)=>void;history:(d:Demand)=>void;canEdit:boolean;canApprove:boolean;onEdit:(d:Demand)=>void;isClient:boolean;clients:any[]}){
  const [openActionId,setOpenActionId]=useState<string|null>(null); const [actionMenuPosition,setActionMenuPosition]=useState<{top:number;left:number}|null>(null);
  useEffect(()=>{
  ...
  <div className="hf-hours-tooltip">
    ...
    <td>{d.responsavel||'—'}</td>
    <td><div className="hf-action-menu" onClick={e=>e.stopPropagation()} style={{position:"relative",display:"inline-block",zIndex:1001}}><button type="button" className="hf-action-btn" onClick={(e)=>{const rect=e.currentTarget.getBoundingClientRect();setActionMenuPosition({top:Math.max(8,rect.top),left:Math.max(8,rect.left-150-8)});setOpenActionId(openActionId===d.id?null:d.id)}} title="Ações"><span>⋮</span><span>Ações</span></button>{openActionId===d.id&&<div style={{position:"fixed",top:actionMenuPosition?.top??0,left:actionMenuPosition?.left??0,zIndex:99999,width:170,display:"flex",flexDirection:"column",gap:4,background:"#fff",border:"1px solid #e5e7eb",borderRadius:10,boxShadow:"0 12px 30px rgba(0,0,0,.16)",padding:6}}><button type="button" className="hf-action-btn" style={{width:"100%",display:"flex",justifyContent:"flex-start",alignItems:"center"}} onClick={()=>{history(d);setOpenActionId(null)}}><History size={15}/><span>Histórico</span></button>{isClient&&<button type="button" className="hf-action-btn primary" style={{width:"100%",display:"flex",justifyContent:"flex-start",alignItems:"center"}} onClick={()=>{onEdit(d);setOpenActionId(null)}}><Eye size={15}/><span>Visualizar</span></button>}{canEdit&&<button type="button" className="hf-action-btn primary" style={{width:"100%",display:"flex",justifyContent:"flex-start",alignItems:"center"}} onClick={()=>{onEdit(d);setOpenActionId(null)}}><Clipboard size={15}/><span>Editar</span></button>}{canEdit&&<button type="button" className="hf-action-btn danger" style={{width:"100%",display:"flex",justifyContent:"flex-start",alignItems:"center"}} onClick={()=>{remove(d.id);setOpenActionId(null)}}><Trash2 size={15}/><span>Excluir</span></button>}</div>}</div></td>
        ...
        <div className="hf-saphire-ia-head">
          <div className="hf-saphire-ia-brand">
            <div className="hf-saphire-ia-gem">
              <img src="/saphire-sheet-favicon.svg" alt="Saphire" />
            </div>
...
.hf-saphire-ia-demand-summary > div{
  min-width:0;
  min-height:58px;
  box-sizing:border-box;
  display:flex;
...
.hf-saphire-ia-demand-summary > div:hover{
  border-color:#d5def5;
  background:#fff;
  box-shadow:0 4px 12px rgba(31,45,75,.05);
}

.hf-saphire-ia-demand-summary > div span{
  color:#7d899b;
  font-size:10px;
  font-weight:700;
  line-height:1;
...
.hf-saphire-ia-demand-summary > div strong{
  color:#1c2940;
  font-size:14px;
  font-weight:800;
  line-height:1.1;
...
.hf-saphire-ia-demand-summary > div:nth-child(3){
  grid-column:span 2;
  background:linear-gradient(135deg,#f0f4ff,#f8faff);
  border-color:#d9e2ff;
}

.hf-saphire-ia-demand-summary > div:nth-child(3) strong{
  color:#315efb;
  font-size:18px;
}

.hf-saphire-ia-demand-summary > div:nth-child(4) strong{
  color:#5f4bb6;
}

.hf-saphire-ia-demand-summary > div:nth-child(5){
  grid-column:span 2;
  display:flex;
  flex-direction:row;
  align-items:center;
...
.hf-saphire-ia-demand-summary > div:nth-child(5) strong{
  color:#344158;
}
...
.hf-saphire-ia-welcome-mini img{
  width:30px;
  height:30px;
  object-fit:contain;
  display:block;
...
@media(max-width:1000px){
  ...
  .hf-sidebar{
    ...
    padding:0 !important;
    box-shadow:18px 0 45px rgba(4,12,24,.24);
    overflow-y:auto;
    overflow-x:hidden;
  }
  ...
  .hf-brand-logo{
    ...
    height:148px;
    padding:22px 42px 14px 22px !important;
    display:flex;
    align-items:center;
    justify-content:center;
    flex-direction:column;
  ...
  .hf-logout{
    margin:4px 14px 16px !important;
    min-height:42px;
    border-radius:10px;
  }
...
@media(max-width:600px){
  ...
  .hf-meu-dia-grid > .hf-panel{
    width:100% !important;
    min-width:0 !important;
    max-width:100% !important;
  }

  .hf-meu-dia-grid .hf-panel-title{
    min-width:0;
  }

  .hf-meu-dia-grid .hf-panel-title > div{
    min-width:0;
    max-width:100%;
  }

  .hf-meu-dia-grid .hf-panel-title h2{
    font-size:15px;
    line-height:1.35;
  }

  .hf-meu-dia-grid .hf-panel-title p{
    font-size:11px;
  }

  /* Lista de demandas dentro de "Precisa da sua atenção". */
  .hf-meu-dia-grid .hf-panel > div[style*="flex-direction"]{
    width:100%;
    min-width:0;
  }
  ...
  .hf-upcoming-item > strong{
    font-size:11px;
    color:#315efb;
  }

  .hf-upcoming-item > div{
    width:100%;
    min-width:0;
  }

  .hf-upcoming-item > div > strong{
    display:block;
    font-size:13px;
    line-height:1.45;
  ...
  .hf-upcoming-item > div > div{
    font-size:11px !important;
    line-height:1.4;
  }

  .hf-upcoming-item > span{
    justify-self:start;
    font-size:11px;
  }
  ...
  .hf-demand-card-footer small{
    font-size:8.5px;
  }

  .hf-demand-card-footer strong{
    font-size:9.5px;
  }
